...
pardir = '../experiment_text_files/big_train_set_files/json_and_csv/1classVS1class'
...
ax = sns.boxplot(x=feats[0], y="instrument", data=df_feat,whis=np.inf)
sns.stripplot(x=feats[0], y="instrument", data=df_feat,jitter=True, size=3, color=".3", linewidth=0)
ax.set_title('accuracy rate:{acc}%'.format(acc = accuracy*100),fontsize = 28)
ax.set_xlabel('effective duration',fontsize = 40)
ax.set_ylabel('')
ax.set(xticklabels=[])
